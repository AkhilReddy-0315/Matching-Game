...
class GameFrame extends Component {
  ...
  getCategoryListedImages = () => {
    const {activeId} = this.state

    const Result = imagesList.filter(eachItem => eachItem.category === activeId)
  ...
  getThumbId = id => {
    ...
    if (id === imagesList[value].id && timer > 0) {
  ...
  render() {
    const {mode, value, activeId, timer, count} = this.state

    const categoryListedImages = this.getCategoryListedImages()
        ...
        {mode ? (
          <div className="game-inner-con">
            ...
            <ul className="images-ul">
              {categoryListedImages.map(eachItem => (
                <ImageItems
                  getThumbId={this.getThumbId}
                  key={eachItem.id}
                  imagesList={eachItem}
                />
              ))}
          ...
          <LostUi resetBtn={this.resetBtn} count={count} />
        )}
      </div>
    )
  }
